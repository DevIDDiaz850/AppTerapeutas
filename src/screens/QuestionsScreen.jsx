...
const OnboardingTest = ({ navigation }) => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3500/getQuestions')
      .then(response => {
  ...
  const Slide = ({ item }) => {
    return (
      <View style={styles.slide}>
        ...
        <Text style={{color:'#fff'}}>Selecciona del 1 al 5 con el Slider de abajo</Text>
        <Slider
          style={{ width: '100%', height: 40 }}
          minimumValue={1}
          maximumValue={5}
          step={1}
          minimumTrackTintColor="#0FCCCE"
